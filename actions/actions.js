...
const normalizeKey = require('@core/memory/keyNormalizer');

const { setContext, getContext, clearContext } = require('@core/memory/contextMemory');
const { introduceSelf } = require('@core/personality/identity');
const { resolveDateRange } = require('@utils/dateResolver');
const { recallByMeaning } = require('@core/memory/recallRouter');
  ...
  switch (intentObj.intent) {
    ...
    case "LOCAL_SKILL":
      ...
      if (intentObj.skill === "WEATHER") {
        const { getWeather } = require('@actions/localSkills');
        return await getWeather(intentObj.city);
      }

      if (intentObj.skill === "NEWS") {
        const { getNews } = require('@actions/localSkills');
