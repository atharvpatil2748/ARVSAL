...
NO MULTILINE STRINGS. NO CODE FENCES.

Your job:
Convert user request into TOOL ACTIONS using EXACT schemas.

You MUST follow examples exactly. Output ONLY a single, valid JSON object.
...
10. NO NEWLINES IN STRINGS. ALL OUTPUT MUST BE ON A SINGLE LINE.
...
{"goal":"short goal","steps":[{"tool":"...","action":"...","params":{}}],"risk":"low"}
`;
}

/* ================= SANITIZER ================= */

function sanitizePlannerJSON(rawText) {
  if (!rawText || typeof rawText !== "string") return "";

  // 1. Remove markdown code fences
  let sanitized = rawText.replace(/```[a-z]*\n?/gi, "").replace(/```/g, "");

  // 2. Collapse ALL literal newlines to spaces. 
  // This repairs unescaped newlines inside quotes AND collapses accidental line breaks.
  sanitized = sanitized.replace(/\r?\n/g, " ");

  // 3. Extract robustly (first { to last })
...
function safeParseJSON(text) {
  if (!text || typeof text !== "string") return { sanitized: null, jsonText: null, parsed: null };

  const sanitized = sanitizePlannerJSON(text);
  const jsonText = sanitized;
  let parsed = null;
  ...
  return { sanitized, jsonText, parsed };
}
...
async function generatePlan({ userInput, worldContext }) {
  ...
  try {
    ...
    const { sanitized, jsonText, parsed } = safeParseJSON(raw);

    console.log("[PLANNER][RAW]", raw);
    console.log("[PLANNER][SANITIZED]", sanitized);
    console.log("[PLANNER][JSON_TEXT]", jsonText);
...
// const { runLLM } = require('@core/reasoning/llmRunner');
// const { askGemini } = require('@core/reasoning/geminiClient');
// const { getActiveAI } = require('@core/reasoning/aiSwitch');
