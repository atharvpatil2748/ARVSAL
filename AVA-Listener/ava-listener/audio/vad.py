...
class HybridVAD:
    ...
    def __init__(self):
        ...
        onnx_path = os.path.join(MODELS_DIR, "silero_vad.onnx")
        if not os.path.isfile(onnx_path):
            log.warning("Silero VAD model not found at %s. Falling back to WebRTC-only.", onnx_path)
            self._silero_sess = None
        else:
            # CPU Execution provider for lightweight low-latency inference
            sess_options = ort.SessionOptions()
            sess_options.inter_op_num_threads = 1
            sess_options.intra_op_num_threads = 1
            
            self._silero_sess = ort.InferenceSession(
                onnx_path, 
        ...
        # Adaptive noise floor tracking for low-energy gating.
