...
const WHISPER_HALLUCINATIONS = new Set([
  "you", "thank you", "thanks", "hmm", "um", "uh", "okay", "ok",
  "bye", "yes", "no", "the", "a", "i", "sure", "right", "alright",
  "hello", "hi", "hey", "oh", "ah", "well"
]);
...
function validateWhisperOutput(raw, stageId = "?") {
  ...
  // Gate 4: word count + hallucination blocklist
  const words = text.split(/\s+/).filter(Boolean);
  if (words.length < 2) {
    const normalized = text.toLowerCase().replace(/[^a-z]/g, "");
    if (WHISPER_HALLUCINATIONS.has(normalized)) {
      console.log(`[${stageId}] ❌ REJECT: hallucination "${text}"`);
      return "";
    }
    // Single meaningful word — allow through
  }

  // Gate 5: strip wake word residue at STT boundary
...
app.post("/audio", async (req, res) => {
  ...
  try {
    ...
    const raw = await runHeavyInference('audio/small', () => runSmallWhisper(wavPath));
    safeDelete(wavPath);
...
app.post("/audio/stream", async (req, res) => {
  ...
  try {
    ...
    const raw = await runSmallWhisper(wavPath);
    safeDelete(wavPath);
