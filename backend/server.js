...
// Start the persistent VAD worker for /audio/stream.
// Fire-and-forget — failure falls back to spawn-per-request silently.
...
async function speakLocally(text) {
  return new Promise((resolve) => {
    try {
      ...
      const piperExe = process.env.ARVSAL_PIPER_PATH || path.resolve(__dirname, "../runtime/piper/piper.exe");

      const modelPath = process.env.ARVSAL_PIPER_MODEL || path.resolve(__dirname, "../runtime/piper/models/en_US-ryan-high.onnx");
...
/* ================= AUDIO — Fallback (Small Model) ================= */

app.post("/audio", async (req, res) => {
  const stageId = `audio_${Date.now()}`;
  try {
    ...
    console.log("[AUDIO][INPUT_SIZE]", webmStats.size);
    console.log("[AUDIO][PATH]", webmPath);
    ...
    console.log(`[${stageId}] ffmpeg_done wavSize=${wavStats.size}`);

    if (wavStats.size < 40000) {
      console.log(`[${stageId}] ❌ REJECT: wav too small (${wavStats.size} bytes)`);
      safeDelete(wavPath);
      return res.json({ text: "" });
    }

    // ── VAD Gate (spawn-per-request) ────────────────────────────────────────
    const vadResult = await vadManager.checkSpeech(wavPath, { mode: 'spawn' });
    console.log(`[VAD][${stageId}] pass=${vadResult.pass} maxProb=${vadResult.maxProb ?? 'n/a'} speechDur=${vadResult.speechDurationMs ?? 'n/a'}ms`);
    if (!vadResult.pass) {
      console.log(`[${stageId}] ❌ VAD REJECT: no qualified speech detected`);
      safeDelete(wavPath);
      return res.json({ text: "" });
    }

    // ── Silence Trim ────────────────────────────────────────────────────────
    let whisperInputPath = wavPath;
    if (vadResult.speechStartMs != null && vadResult.speechEndMs != null) {
      ...
      if (trimOk) {
        ...
        console.log(`[${stageId}] ✂️  Silence trimmed: ${trimStart.toFixed(2)}s–${trimEnd.toFixed(2)}s`);
      } else {
        safeDelete(trimmedPath);
        console.warn(`[${stageId}] ⚠️  Trim failed — using original WAV`);
      }
    }

    const raw = await runHeavyInference('audio/small', () => runSmallWhisper(whisperInputPath));
    safeDelete(whisperInputPath);

    const text = validateWhisperOutput(raw, stageId);

    // Reset streaming state on full audio submission
    streamFullText = "";
    lastStreamTime = 0;
    ...
    console.error(`[${stageId}] AUDIO ERROR:`, err.message);
    return res.json({ error: "Audio processing failed", details: err.message });
  }
}
);
...
app.post("/audio/final", async (req, res) => {
  ...
  try {
    ...
    console.log(`[${stageId}] [INSTRUMENT] webm path: ${webmPath}`);
    console.log(`[${stageId}] [INSTRUMENT] wav path: ${wavPath}`);
    ...
    await new Promise((resolve, reject) => {
      ...
      console.log("[AUDIO][PATH]", webmPath);
      console.log("[FFMPEG][CMD]", ffmpegExe, args.join(" "));
      console.log(`[${stageId}] [INSTRUMENT] ffmpeg start (CMD: ${ffmpegExe} ${args.join(" ")})`);
      const ff = spawn(ffmpegExe, args);
      let stderr = "";
      ff.stderr.on("data", data => { stderr += data.toString(); });
      ff.on("close", code => {
    ...
    // ── VAD Gate (spawn-per-request) ────────────────────────────────────────
    console.log(`[${stageId}] [INSTRUMENT] VAD request start on ${wavPath}`);
    const vadResult = await vadManager.checkSpeech(wavPath, { mode: 'spawn' });
    console.log(`[${stageId}] [INSTRUMENT] VAD result: ${JSON.stringify(vadResult)}`);
    console.log(`[VAD][${stageId}] pass=${vadResult.pass} maxProb=${vadResult.maxProb ?? 'n/a'} speechDur=${vadResult.speechDurationMs ?? 'n/a'}ms`);
    if (!vadResult.pass) {
      console.log(`[${stageId}] ❌ VAD REJECT: no qualified speech detected`);
      safeDelete(wavPath);
...
/* ================= AUDIO/STREAM — Pseudo-Streaming (Small Model) ================= */
// NOTE: This is pseudo-streaming. Each chunk runs full small-model inference.
// It is NOT true real-time streaming (which requires direct PCM pipe into whisper).
// Real streaming is NOT implemented and NOT planned in this version.

app.post("/audio/stream", async (req, res) => {
  const stageId = `stream_${Date.now()}`;
  try {
    if (!req.body || !req.body.length) {
      return res.json({ partial: "" });
    }

    console.log(`[${stageId}] stream_received bytes=${req.body.length}`);
    ...
    const wavStats = fs.statSync(wavPath);
    if (wavStats.size < 16000) { // lower threshold for streaming chunks
      safeDelete(wavPath);
      return res.json({ partial: "" });
    }

    // ── VAD Gate (persistent worker) ────────────────────────────────────────
    const vadResult = await vadManager.checkSpeech(wavPath, { mode: 'persistent' });
    console.log(`[VAD][${stageId}] pass=${vadResult.pass} maxProb=${vadResult.maxProb ?? 'n/a'} speechDur=${vadResult.speechDurationMs ?? 'n/a'}ms`);
    if (!vadResult.pass) {
      console.log(`[${stageId}] ❌ VAD REJECT: no qualified speech in stream chunk`);
      safeDelete(wavPath);
      return res.json({ partial: "" });
    }

    // ── Silence Trim (stream chunks — trim if boundaries available) ──────────
    let whisperInputPath = wavPath;
    if (vadResult.speechStartMs != null && vadResult.speechEndMs != null) {
      const trimmedPath = createTempFile("vad_trimmed", ".wav");
      const ffmpegExeTrim = process.env.ARVSAL_FFMPEG_PATH || path.resolve(__dirname, "../runtime/ffmpeg/bin/ffmpeg.exe");
    ...
    const raw = await runSmallWhisper(whisperInputPath);
    safeDelete(whisperInputPath);

    const partial = validateWhisperOutput(raw, stageId);
    console.log(`[${stageId}] stream_partial="${partial}"`);

    return res.json({ partial: partial || "" });

  } catch (err) {
    console.error(`[${stageId}] STREAM ERROR:`, err.message);
    return res.json({ partial: "" });
  }
});
