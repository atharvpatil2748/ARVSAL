/**
 * Reflection Engine (COMPATIBILITY WRAPPER)
 *
 * 🔒 IMPORTANT:
 * - This file NO LONGER generates reflections directly
 * - Delegates to Phase-3 reflection pipeline
 * - Preserves legacy API to avoid breaking imports
 */
 ...
 * Uses real trigger logic internally
 */
function shouldReflect(subject = "user") {
  return !!shouldTriggerReflection(subject);
}


/**
 * Legacy-compatible reflection runner
 * Delegates to signal-based reflection system
 */
async function reflect(subject = "user") {
  const signal = shouldTriggerReflection(subject);
  if (!signal) return;

  try {
    const reflection = await generateReflection(signal);
    if (!reflection || !reflection.summary) return;
